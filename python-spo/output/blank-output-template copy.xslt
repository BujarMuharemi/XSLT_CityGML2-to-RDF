<?xml version="1.0" encoding="UTF-8"?>
  ...
  <!-- ROOT element-->
  <xsl:template match="/CityModel">
    <xsl:variable name="id" select="f:slugify(@gml:id)" />

    <CityModel rdf:about="{concat('CityModel/', $id)}">
      <schema:identifier>
...
<!-- base template for triples -->
  <xsl:template match="bldg:Building/gml:boundedBy/gml:Envelope">
    <xsl:variable name="id" select="f:slugify(@gml:id)" />

    <gml:boundedBy>
      <gml:Envelope rdf:about="{concat('gml:Envelope/', $id)}">
        <schema:identifier>
          <xsl:value-of select="concat('gml:Envelope.',$id)" />
        </schema:identifier>
...
<!-- base template for triples -->
  <xsl:template match="bldg:RoofSurface/gen:stringAttribute/gen:value">
    ...
    <gen:stringAttribute>
      <gen:value rdf:about="{concat('gen:value/', $id)}">
        <schema:identifier>
          <xsl:value-of select="concat('gen:value.',$id)" />W
        </schema:identifier>
